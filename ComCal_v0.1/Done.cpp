...
std::string Done::executeInput(std::string inputString) {
	...
	for (unsigned int i = 0; i < _doneTaskIndexes.size(); i++) {
		if (contents[_doneTaskIndexes[i]].getCode() == TASKTYPECODE_INVALID) {
			throw exceptionInputTaskAlreadyRemoved;
		}

		if (contents[_doneTaskIndexes[i]].getCode() == TASKTYPECODE_FLOATING || contents[_doneTaskIndexes[i]].getCode() == TASKTYPECODE_DEADLINE) {
...
std::string Done::undo(void) {
	...
	for (unsigned int i = 0; i < doneTasks.size(); i++) {
		...
		for (unsigned int j = 0; j < todoContents.size() && !isFoundTodo; j++) {
			if (todoContents[j].getCode() == TASKTYPECODE_INVALID && todoContents[j].getDescription() == doneTasks[i].getDescription() && todoContents[j].getStartDateTime() == doneTasks[i].getStartDateTime() && todoContents[j].getEndDateTime() == doneTasks[i].getEndDateTime() && todoContents[j].getLocation() == doneTasks[i].getLocation()) {
		...
		for (unsigned int k = 0; k < doneContents.size() && !isFoundDone; k++) {
			if (doneContents[k].getCode() != TASKTYPECODE_INVALID && doneContents[k].getDescription() == doneTasks[i].getDescription() && doneContents[k].getStartDateTime() == doneTasks[i].getStartDateTime() && doneContents[k].getEndDateTime() == doneTasks[i].getEndDateTime() && doneContents[k].getLocation() == doneTasks[i].getLocation()) {
...
std::string Done::redo(void) {
	...
	for (unsigned int i = 0; i < doneTasks.size(); i++) {
		...
		for (unsigned int j = 0; j < doneContents.size() && !isFoundDone; j++) {
			if (doneContents[j].getCode() == TASKTYPECODE_INVALID && doneContents[j].getDescription() == doneTasks[i].getDescription() && doneContents[j].getStartDateTime() == doneTasks[i].getStartDateTime() && doneContents[j].getEndDateTime() == doneTasks[i].getEndDateTime() && doneContents[j].getLocation() == doneTasks[i].getLocation()) {
		...
		for (unsigned int k = 0; k < todoContents.size() && !isFoundTodo; k++) {
			if (todoContents[k].getCode() != TASKTYPECODE_INVALID && todoContents[k].getDescription() == doneTasks[i].getDescription() && todoContents[k].getStartDateTime() == doneTasks[i].getStartDateTime() && todoContents[k].getEndDateTime() == doneTasks[i].getEndDateTime() && todoContents[k].getLocation() == doneTasks[i].getLocation()) {
